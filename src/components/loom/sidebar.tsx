...
import { BrainCircuit, Camera, DollarSign, KeyRound, ShieldCheck, Trash2, Microscope, Bot, TestTube2 } from "lucide-react";
  ...
  return (
    <aside className="w-80 shrink-0 bg-card/30 border-r-2 border-gilded-accent/20 glow-gilded flex flex-col">
      ...
      <ScrollArea className="flex-1">
        <Accordion type="multiple" defaultValue={['dna', 'snapshots']} className="w-full">
            <AccordionItem value="dna" className="border-b-0">
                <Card className="rounded-none border-0 border-b border-border/50 bg-transparent">
                    <CardHeader className="p-0">
                        <AccordionTrigger className="p-4 font-headline text-lg hover:no-underline">Agent DNA</AccordionTrigger>
                    </CardHeader>
                    <AccordionContent className="p-4 pt-0 space-y-4">
                        <div className="flex justify-between items-center">
                            <CardTitle className="text-base flex items-center gap-2"><Bot/>{agentData.name}</CardTitle>
                            <Badge variant="outline" className="text-accent border-accent">{agentData.certification}</Badge>
                        </div>
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {agentData.dna.coreCapabilities.map(cap => (
                                    <Badge key={cap.id}><cap.icon className="w-3 h-3 mr-1.5"/>{cap.name}</Badge>
                                ))}
                            </div>
                        </div>
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {agentData.dna.permissions.map(perm => (
                                     <Badge variant="secondary" key={perm.id}><perm.icon className="w-3 h-3 mr-1.5"/>{perm.name}</Badge>
                                ))}
                            </div>
                        </div>
                    </AccordionContent>
            ...
            <AccordionItem value="fates" className="border-b-0">
                <Card className="rounded-none border-0 border-b border-border/50 bg-transparent">
                    <CardHeader className="p-0">
                        <AccordionTrigger className="p-4 font-headline text-lg hover:no-underline">Engine Tuning</AccordionTrigger>
                    </CardHeader>
                    <AccordionContent className="p-4 pt-0">
                        <CardDescription className="mb-4">Tune the KLEPSYDRA Engine's parameters.</CardDescription>
            ...
            <AccordionItem value="snapshots" className="border-b-0">
                    ...
                    <AccordionContent className="p-4 pt-0 space-y-3">
                        ...
                        <div className="space-y-2">
                            {snapshots.map(snapshot => (
                                <div key={snapshot.id} className="flex items-center justify-between p-2 rounded-md bg-muted/50 hover:bg-muted transition-colors">
                                    <div>
             ...
             <AccordionItem value="experiments" className="border-b-0">
                <Card className="rounded-none border-0 bg-transparent">
                    <CardHeader className="p-0">
                        <AccordionTrigger className="p-4 font-headline text-lg hover:no-underline">Experiments</AccordionTrigger>
                    </CardHeader>
                    <AccordionContent className="p-4 pt-0 space-y-3">
